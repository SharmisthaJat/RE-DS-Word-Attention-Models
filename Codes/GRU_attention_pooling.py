...
class PCNN(nn.Module):
    def __init__(self, word_length, feature_length, cnn_layers, Wv, pf1, pf2, kernel_size, word_size=50, feature_size=5, dropout=0.5, num_classes=53, num_words=82):
        ...
        self.linear = nn.Linear(self.cnn_layers*6, 53)
...
def trainModel(train, test, dev, epochs, directory, Wv, pf1, pf2, batch=50, num_classes=53, max_sentences=5, img_h=82, to_train=1, test_epoch=0):
    ...
    for epoch in range(test_epoch,epochs):
        ...
        test_pr_max = pr_max(test_predict[3], test_predict[2], test_entity)
        ...
        if to_train == 1:
            ...
            out_max = open(directory+"pr_max_"+str(epoch),"wb")
        else:
            out = open(directory+"pr_all_"+str(epoch),"wb")
            out_max = open(directory+"pr_temp_max_"+str(epoch),"wb")
        for res in test_pr[3]:
            out.write(str(res[0])+"\t"+str(res[1])+"\n")
        for res in test_pr_max[3]:
            out_max.write(str(res[0])+"\t"+str(res[1])+"\n")
        out.close()
        out_max.close()
        now = time.strftime("%Y-%m-%d %H:%M:%S")
        precision = -1
        ...
        print str(now) + '\t epoch ' + str(epoch) + "\tTest\tScore:"+str(score)+"\t Precision : "+str(test_pr_max[0]) + "\t Recall: "+str(test_pr_max[1])+ "\t Total: "+ str(test_pr_max[2])
        now = time.strftime("%Y-%m-%d %H:%M:%S")
...
def get_attention_scores(label, sents, pos, ldist, rdist, epos, entity, img_h , numClasses, maxSentences, testModel, filterSize = 3, batch = 1000):
    ...
    cPickle.dump(attention_scores_final, open('attention_scores_final_gru.p','wb'), -1)
...
def pr(predict_y, true_y,entity_pair):
    ...
    f_ = open('GRUAttention_test.csv','w')
    for g,item in enumerate(resultSorted):
...
if __name__ == "__main__":
    ...
    resultdir = inputdir
    resultdir = "/scratchd/home/siddesh/DocumentTimestamp/PCNN+ATT/GRU_Attention_final_"+resultdir
