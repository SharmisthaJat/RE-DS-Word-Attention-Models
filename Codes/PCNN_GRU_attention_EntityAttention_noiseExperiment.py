...
class PCNN(nn.Module):
    def __init__(self, word_length, feature_length, cnn_layers, Wv, pf1, pf2, kernel_size, word_size=50, feature_size=5, dropout=0.5, num_classes=53, num_words=82):
        ...
        self.linear = nn.Linear(self.cnn_layers*9 + (self.word_size + 2*self.feature_size)*6, self.num_classes)
...
def get_attention_scores(label, sents, pos, ldist, rdist, epos, entity, img_h , numClasses, maxSentences, testModel, filterSize = 3, batch = 1000):
    ...
    for bagIndex, insRel in enumerate(label):
        ...
        for m in range(insNum):
            ...
            insEntity.append(entity[bagIndex])
    ...
    cPickle.dump(attention_scores_final, open('attention_scores_final_google.p','wb'), -1)
...
if __name__ == "__main__":
    ...
    resultdir = inputdir
    for i in range(1,2):
        resultdir = "/scratchd/home/siddesh/DocumentTimestamp/PCNN+ATT/bag_variation_results_fixed/"+str(i)+'/'
        ...
        trainModel(train,
                        ...
                        Wv,
                        PF1,
                        PF2,batch=50, test_epoch=156, num_classes=5, to_train=0)
