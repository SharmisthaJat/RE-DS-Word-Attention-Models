...
class EntitySpecificEmbeddingSmall(nn.Module):
    def __init__(self):
        super(EntitySpecificEmbeddingSmall, self).__init__()
...
class PCNN(nn.Module):
    def __init__(self, word_length, feature_length, cnn_layers, Wv, pf1, pf2, kernel_size, word_size=50, feature_size=5, dropout=0.5, num_classes=53, num_words=82):
        ...
        self.entityEmbedding = EntitySpecificEmbeddingSmall()
        self.cnn = CNNwithPool(self.cnn_layers, self.kernel_size)
        self.drop = nn.Dropout(dropout)
        self.linear = nn.Linear(self.cnn_layers*9 + (self.word_size + 2*self.feature_size)*6, 53)
    ...
    def forward(self, x, ldist, rdist, pool):
        ...
        sentence_embedding = F.tanh(torch.cat((cnn, pieceWisePool, entity1_pool, entity2_pool), cnn.dim() -1))
        cnn_dropout = self.drop(sentence_embedding)
        probabilities = self.linear(cnn_dropout)
        return probabilities, [attention_scores, entity1_attention_values, entity2_attention_values]
...
def trainModel(train, test, dev, epochs, directory, Wv, pf1, pf2, batch=50, num_classes=53, max_sentences=5, img_h=82, to_train=1, test_epoch=0):
    model = PCNN(word_length=len(Wv), feature_length=len(pf1), cnn_layers=230, kernel_size=(3,60), Wv=Wv, pf1=pf1, pf2=pf2)
    ...
    for epoch in range(test_epoch,epochs):
        ...
        if to_train == 1:
            ...
            cPickle.dump(test_predict,open(directory+"predict_prob_all_"+str(epoch),"wb"))
        print "Test"
        ...
        if to_train == 1:
            ...
            out = open(directory+"pr_all_"+str(epoch),"wb")
...
def pr(predict_y, true_y,entity_pair):
    ...
    dictionary = cPickle.load(open('/scratchd/home/siddesh/DocumentTimestamp/PCNN+ATT/NRE_processed/dictionary.p'))
    f_ = open('BNETDS_test.csv','w')
    for g,item in enumerate(resultSorted):
        f_.write(str(item[1]) + ":::"+str(final_labels[item[0]])+ ":::" + str(item[2])+":::"+str([dictionary[entt + 1] for entt in item[3]])+"\n")
    f_.close()
    p_p = 0.0
    p_n = 0.0
    n_p = 0.0
...
def get_attention_scores(label, sents, pos, ldist, rdist, epos, entity, img_h , numClasses, maxSentences, testModel, filterSize = 3, batch = 1000):
    ...
    for batch_index, (batch_start, batch_end) in enumerate(batches):
        ...
        for idd in range(len(batch_ids)):
            attention_scores_all.append([attention_scores[0][idd,:,0],attention_scores[1][idd,:,0],attention_scores[2][idd,:,0]])
    ...
    cPickle.dump(attention_scores_final, open('attention_scores_final.p','wb'), -1)
...
if __name__ == "__main__":
    inputdir = "NRE_processed/"
    resultdir = inputdir
    resultdir = "/scratchd/home/siddesh/DocumentTimestamp/PCNN+ATT/PCNN_GRU_attention_EntityAttention_final_fixed_"+resultdir
    print 'result dir='+resultdir
    if not os.path.exists(resultdir):
        os.mkdir(resultdir)

    dataType = "_features_all_6Months"
    test = cPickle.load(open(inputdir+'/test_full_final_3_80.p'))
    train = cPickle.load(open(inputdir+'/train_3_80.p'))
    # train = test
    # random.shuffle(test)
    # dev = test[:10000]
    ...
    trainModel(train,
                    ...
                    Wv,
                    PF1,
                    PF2,batch=50, test_epoch=25, to_train=0)
